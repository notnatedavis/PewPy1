...
class AimbotWorker(BaseWorker) :
    ...
    #
    # NEW: Mouse control can be independently enabled/disabled via set_mouse_control().
    # When disabled, the activation key does nothing and the mouse never moves.
    ...
    def __init__(self,
        ...
        # NEW: master switch for mouse movement
        self.mouse_control_enabled = False

        # Force output_color="RGB" to match hue detection scan
        self.capturer = ScreenCapturer(region=capture_region, target_fps=target_fps, output_color="RGB")
        ...
        # store latest detection stats for debug
        self._last_contour_count = 0
        self._last_best_confidence = 0.0
        self._last_reject_reason = ""

        logging.info(f"Aimbot initialized - Region: {capture_region}, FPS: {target_fps}")

    def _on_key_press(self, key) -> None :
        # Ignore key if mouse control is disabled
    ...
    # ----- New setter for mouse control -----
    def set_mouse_control(self, enabled: bool) -> None :
        """Enable or disable the mouse movement feature independently."""
        with self.state_lock :
            self.mouse_control_enabled = enabled
            if not enabled:
                # Clear any pending aiming state to avoid accidental movement
                self._aiming_enabled.clear()
            logging.info(f"Mouse control {'enabled' if enabled else 'disabled'}")
    ...
    def _process_frame(self) -> None :
        ...
        if frame is None :
            ...
            return
        self.frame_count += 1
        logging.debug(f"Aimbot: frame captured, shape={frame.shape}, frame_count={self.frame_count}")
        ...
        if detection_frame is not None :
            ...
            logging.debug(f"Aimbot: detection HSV range (user): lower=({lower_user[0]}°,{lower_user[1]}%,{lower_user[2]}%) "
                          f"upper=({upper_user[0]}°,{upper_user[1]}%,{upper_user[2]}%)")
            detection = self.detector.detect_targets(detection_frame)
            best_candidate = self.detector.get_best_candidate()

            # estimate contour count from mask (we can get latest mask and find contours)
            ...
            if best_candidate :
                best_conf = best_candidate.get('confidence', 0.0)
            else :
                best_conf = 0.0

            if detection is not None :
                logging.debug(f"Aimbot: Detection found! Confidence={detection.get('confidence',0):.2f}, "
                              f"center={detection['center']}, contour pts={len(detection.get('contour', []))}")
                if roi_offset != (0,0):
                ...
                if best_candidate is None :
                    reject_reason = "No candidate contours found (area/bbox empty?)"
                else :
                    conf = best_candidate.get('confidence', 0.0)
                    if conf < self.detector.min_confidence:
                        reject_reason = f"Best confidence {conf:.3f} below threshold {self.detector.min_confidence:.3f}"
                    else :
                        # Possibly area filter
                        reject_reason = "Candidate rejected by area limits or other filters"
                logging.debug(f"Aimbot: No detection. {reject_reason}")
        else :
            logging.debug("Aimbot: detection_frame is None, skipping detection")
            reject_reason = "ROI region invalid or frame None"

        # store debug stats
        self._last_contour_count = contour_count
        self._last_best_confidence = best_conf
        ...
        if self.overlay_data is not None :
            ...
            if detection is not None :
                ...
                logging.debug(f"Aimbot: sending to overlay_data: target_center={data_to_send['target_center']}, "
                              f"contour points={len(data_to_send['target_contour']) if data_to_send['target_contour'] else 0}")
            else : 
                data_to_send = {'target': None, 'target_center': None, 'bbox': None, 'target_contour': None}
                logging.debug("Aimbot: sending empty data to overlay_data (no detection)")
            if mask is not None :
        ...
        if self.debug_queue is not None:
            ...
            if now - self._last_debug_time >= self._debug_interval :
                ...
                debug_data = self._collect_debug_info(detection, best_candidate, mouse_pos, contour_count, best_conf, reject_reason)
                try :
                    self.debug_queue.put_nowait({'type': 'detection_debug', 'data': debug_data})
                except queue.Full:
                    pass  # drop update if queue is full

        # --- Mouse movement: only if detection exists, aiming enabled, AND mouse control enabled ---
        if detection is not None and self._aiming_enabled.is_set() and self.mouse_control_enabled :
            self._aim_at_target(detection)

    def _collect_debug_info(self, detection: Optional[Dict], best_candidate: Optional[Dict],
                            mouse_pos: Optional[Tuple[int,int]], contour_count: int,
                            best_conf: float, reject_reason: str) -> Dict:
        # Collect current HSV bounds (user-friendly), detection status, candidate info, and debug flags
        lower_cv = self.detector.lower_hsv
        upper_cv = self.detector.upper_hsv
        lower_user = opencv_hsv_to_user(int(lower_cv[0]), int(lower_cv[1]), int(lower_cv[2]))
        upper_user = opencv_hsv_to_user(int(upper_cv[0]), int(upper_cv[1]), int(upper_cv[2]))
        ...
        info = {
            ...
            'aimbot_running': self._status.value == 'RUNNING',  # from BaseWorker
            'roi_enabled': self.roi_enabled,
            'roi_radius': self.roi_radius,
            'mouse_pos': mouse_pos,
            'confidence_threshold': self.detector.min_confidence,
            'outline_mode': self.detector.outline_mode,
    ...
    def set_hsv_range(self, lower, upper) :
        ...
        logging.debug(f"Aimbot HSV range updated to lower={lower}, upper={upper}")
